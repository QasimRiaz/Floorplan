...
/**
 * 
 */  // This function will allow to preset graph area
PageSetupDialog.addPageFormatPanel = function(div, namePostfix, pageFormat, pageFormatListener)
{
	...
	var widthInputFt = document.createElement('input');
	widthInputFt.setAttribute('size', '6');
	widthInputFt.setAttribute('id', 'widthInputft');
        widthInputFt.setAttribute('readonly', 'true');
	widthInputFt.setAttribute('value', pageFormat.width/mxPixelPerFeet);
	customDiv.appendChild(widthInputFt);
	mxUtils.write(customDiv, ' x ');
	
	var heightInputFt = document.createElement('input');
	heightInputFt.setAttribute('size', '6');
	heightInputFt.setAttribute('id', 'heightInputft');
        heightInputFt.setAttribute('readonly', 'true');
	heightInputFt.setAttribute('value', pageFormat.height/mxPixelPerFeet);
	customDiv.appendChild(heightInputFt);
	mxUtils.write(customDiv, ' ft');
	...
	function listener(sender, evt, force)
	{
		if (force || (widthInput != document.activeElement && heightInput != document.activeElement))
		{
			...
			if (!detected)
			{
				//assign value in feets
				//widthInput.value = widthInputFt.value*mxPixelPerFeet;
				//heightInput.value = heightInputFt.value*mxPixelPerFeet;
	...
	mxEvent.addListener(widthInputFt, 'blur', updateInput);
	mxEvent.addListener(widthInputFt, 'click', updateInput);
	mxEvent.addListener(heightInputFt, 'blur', updateInput);
	mxEvent.addListener(heightInputFt, 'click', updateInput);
...
{
	//console.log(mxPixelPerFeet);
	return [//{key: '50x50', title: 'Default (50ft x 50ft)', format: mxConstants.PAGE_FORMAT_LETTER_PORTRAIT},
