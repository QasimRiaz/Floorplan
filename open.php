...
<script type="text/javascript">
	// Reads files locally
        currentsiteurl = '<?php echo  get_site_url(); ?>';
	function handleFiles(files)
	{
		...
		for (var i = 0; i < files.length; i++)
		{
			(function(file)
			{
				...
				jQuery.ajax({
					url : currentsiteurl+'/wp-content/plugins/floorplan/ajax-call.php',
					data: data,
					cache: false,
					contentType: false,
					processData: false,
					...
					success : function( response ) {
						...
						{
							window.parent.openFile.setData(e.target.result, response);
						};
	...
	function main()
	{
		...
		else
		{
			...
			var openButton = document.getElementById('openButton');
			openButton.value = 'Set Background';//window.parent.mxResources.get(window.parent.openKey || 'open');
...
<input type="file" name="upfile" id="upfile" onchange="fileChanged()">
</td>
</tr>
<tr>
<td colspan="2" id="openSupported" style="font-family:arial;color:grey;font-size:9pt;vertical-align:top;text-align:left;">
</td>
</tr>
...
<td style="vertical-align:middle;white-space:nowrap;">
    <input type="button" id="cancelButton" class="geBtn" value="Cancel" style="margin-left: -6px;" onclick="hideWindow(true);">
<input type="submit" id="openButton" class="geBtn gePrimaryBtn" value="Set Background" disabled="disabled">
</td>
</tr>
</table>
</form>
